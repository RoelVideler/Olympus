#!/usr/bin/env python3
"""Hermes Agent installation and discovery findings.

This script documents the results of systematically exploring Hermes Agent's
capabilities. Run `hermes --help` for the latest CLI reference.
...
#
# Command: hermes acp
# Description: Run Hermes Agent in ACP mode for editor integration
...
#
# Note: ACP dependencies not installed by default.
# Install with: pip install -e '.[acp]'
...
#             clarify, delegation, cronjob, messaging, homeassistant, spotify
...
#
# Plugin system: YES — Hermes has a plugin system
...
#    The kanban system dispatches tasks to profiles via the gateway.
#    Profiles claim tasks atomically and execute in isolated workspaces.
#
# 3. Cross-platform messaging:
#    Profiles can deliver results via Telegram, Discord, Signal, etc.
#    Configured via `hermes cron create --deliver <platform>`.
#
# 4. MCP server:
#    hermes mcp serve exposes Hermes conversations to other agents.
#    Other agents can connect as MCP clients.
#
# 5. ACP server:
#    hermes acp runs Hermes as an ACP server for editor integration.
#
# Profile isolation:
...
#   hermes -p <name> -z "Prompt" -Q  # quiet mode, no banner
#
...
# Categories: apple, autonomous-ai-agents, creative, data-science, devops,
#             email, gaming, github, mcp, media, mlops, note-taking,
...
#
# Tasks are claimed atomically and executed by a named profile
...
def main():
    """Print summary of Hermes Agent discovery findings."""
    print("Hermes Agent Discovery Summary")
    print("=" * 50)
    print()
    print("Version: 0.14.0 (2026.5.16)")
    print("Package: hermes-agent")
    print("CLI command: hermes")
    print()
    print("Key findings:")
    print("  - Profile system: YES (hermes profile)")
    print("  - ACP interface: YES (hermes acp)")
    print("  - MCP interface: YES (hermes mcp)")
    print("  - Tool plugins: YES (hermes tools, hermes plugins)")
    print("  - Cron system: YES (hermes cron)")
    print("  - Skills system: YES (81 builtin skills)")
    print("  - Kanban board: YES (multi-profile collaboration)")
    print("  - Web dashboard: YES (port 9119)")
    print()
    print("Profile storage:")
    print("  Default: ~/.hermes/")
    print("  Named:   ~/.hermes/profiles/<name>/")
    print()
    print("Run a profile:")
    print("  hermes -p <name> chat")
    print("  hermes -p <name> -z \"prompt\"")
    print("  <name> chat  (via wrapper alias)")
    print()
    print("See scripts/setup_hermes.py for detailed findings.")
